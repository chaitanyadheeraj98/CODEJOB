...
class NumberReviewFieldChangesMigrationTests(unittest.TestCase):
    def test_upgrade_adds_column_and_downgrade_removes_it(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with engine.begin() as connection:
                connection.exec_driver_sql("ALTER TABLE number_review_queue DROP COLUMN field_changes_json")
                connection.exec_driver_sql("CREATE TABLE alembic_version (version_num VARCHAR(32) NOT NULL)")
                connection.exec_driver_sql("INSERT INTO alembic_version(version_num) VALUES ('20260906_0044')")
            engine.dispose()
            ...
            try:
                ...
                self.assertIn("field_changes_json", columns)
                with Session(upgraded) as session:
                    self.assertEqual(session.get(NumberReviewQueue, card_id).field_changes_json, "")
                upgraded.dispose()

                command.downgrade(config, "20260906_0044")
