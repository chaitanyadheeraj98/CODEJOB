...
| `backend/app/config.py` | `redis_url = "redis://localhost:6379/0"` | Advertises Redis queue infrastructure even though no live Redis worker currently uses it |
...
| `backend/app/main.py` | default policy batch limits and named profile presets | Controls run aggressiveness |
...
### Queue-infrastructure signaling

Redis and RQ dependencies are configured, but the current branch uses threads, DB rows, and direct service calls instead of a Redis-backed worker queue. This makes `redis_url` and related infra defaults a documentation and expectation-management hotspot.
...
3. **Medium priority:** document and periodically review heuristic keyword lists, default templates, opportunity status values, and the inactive Redis/RQ config.
...
Hardcoded values are not just cosmetic in CODEJOB. Several of them define live workflow behavior, operator experience, or deployment safety, and a few of them advertise infrastructure that is not active in runtime. Any change to these constants should be reflected in tests and documentation in the same branch update.
