...
type TrackingTab = 'gaps' | 'resumes' | 'submissions'

type Props = { apiBase: string; onNavigateToSettings?: (resumeId: number) => void; activeTab?: TrackingTab; onTabChange?: (tab: TrackingTab) => void; filterValues?: FilterValues; sortValue?: string }
...
export default function ResumeTrackingPage({ apiBase, onNavigateToSettings, activeTab, onTabChange, filterValues = {}, sortValue = 'newest' }: Props) {
  ...
  return (
    <section className="card pageSection resumeTrackingPage">
      ...
      {tab === 'gaps' ? <RoleGapsTab apiBase={apiBase} /> : tab === 'resumes' ? <ResumesTab apiBase={apiBase} onNavigateToSettings={onNavigateToSettings} sortValue={sortValue} filterValues={filterValues} /> : <SubmissionsTab apiBase={apiBase} resumes={resumes} filterValues={filterValues} sortValue={sortValue} />}
    </section>
  )
}
