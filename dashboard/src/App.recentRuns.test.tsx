...
async function flushPromises(iterations = 1): Promise<void> {
...
describe('Recent Runs skipped-item drill-down', () => {
  ...
  it('includes mail_date on recent-runs fetch when the selected date is persisted and omits it after clearing', async () => {
    ...
    for (let attempt = 0; attempt < 10 && !fetchUrls.some((url) => url.includes('/recent-runs?')); attempt += 1) {
      await act(async () => {
        await flushPromises(4)
      })
    }

    const initialRecentRunsCall = fetchUrls.filter((url) => url.includes('/recent-runs?')).at(-1)
    expect(initialRecentRunsCall).toBeDefined()
    expect(initialRecentRunsCall).toContain('mail_date=2026-07-08')

    const clearDateButton = Array.from(container.querySelectorAll('button')).find((button) =>
      button.textContent?.includes('Jul 8, 2026'),
    ) as HTMLButtonElement | undefined
    expect(clearDateButton).toBeDefined()

    const fetchCountBeforeClear = fetchUrls.length
    await act(async () => {
      clearDateButton?.dispatchEvent(new MouseEvent('click', { bubbles: true }))
      await flushPromises(3)
    })

    const postClearRecentRunsCalls = fetchUrls
      .slice(fetchCountBeforeClear)
      .filter((url) => url.includes('/recent-runs?'))
    expect(postClearRecentRunsCalls.length).toBeGreaterThan(0)
    expect(postClearRecentRunsCalls[postClearRecentRunsCalls.length - 1]).not.toContain('mail_date=')
  })
})
