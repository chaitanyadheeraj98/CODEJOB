  ...
  it('renders Regenerate, posts to the endpoint, shows the pending label, and refreshes the draft', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.endsWith('/settings/resumes')) return Promise.resolve(makeResponse([]))
        if (url.endsWith('/settings/attachments')) return Promise.resolve(makeResponse([]))
        if (url.endsWith('/settings/skills/pending')) return Promise.resolve(makeResponse([]))
        if (url.endsWith('/settings/job-intent-learning/pending')) return Promise.resolve(makeResponse([]))
        if (url.endsWith('/settings/job-intent-learning/approved')) return Promise.resolve(makeResponse([]))
